...
static void CharLCD_WrDatNib(uint8 nibble) ;
static void CharLCD_WrCntrlNib(uint8 nibble) ;
...
void CharLCD_Init(void) 
{
    ...
    CharLCD_WrCntrlNib(CharLCD_DISPLAY_8_BIT_INIT);    /* Selects 8-bit mode */
    CyDelay(5u);                                                         /* Delay 5 ms */
    CharLCD_WrCntrlNib(CharLCD_DISPLAY_8_BIT_INIT);    /* Selects 8-bit mode */
    CyDelay(15u);                                                        /* Delay 15 ms */
    CharLCD_WrCntrlNib(CharLCD_DISPLAY_8_BIT_INIT);    /* Selects 8-bit mode */
    CyDelay(1u);                                                         /* Delay 1 ms */
    CharLCD_WrCntrlNib(CharLCD_DISPLAY_4_BIT_INIT);    /* Selects 4-bit mode */
    CyDelay(5u);                                                         /* Delay 5 ms */

    CharLCD_WriteControl(CharLCD_CURSOR_AUTO_INCR_ON);    /* Incr Cursor After Writes */
    CharLCD_WriteControl(CharLCD_DISPLAY_CURSOR_ON);      /* Turn Display, Cursor ON */
    CharLCD_WriteControl(CharLCD_DISPLAY_2_LINES_5x10);   /* 2 Lines by 5x10 Characters */
    CharLCD_WriteControl(CharLCD_DISPLAY_CURSOR_OFF);     /* Turn Display, Cursor OFF */
    CharLCD_WriteControl(CharLCD_CLEAR_DISPLAY);          /* Clear LCD Screen */
    CharLCD_WriteControl(CharLCD_DISPLAY_ON_CURSOR_OFF);  /* Turn Display ON, Cursor OFF */
    CharLCD_WriteControl(CharLCD_RESET_CURSOR_POSITION);  /* Set Cursor to 0,0 */
    CyDelay(5u);
...
void CharLCD_WriteData(uint8 dByte) 
{
    ...
    CharLCD_IsReady();
    nibble = dByte >> CharLCD_NIBBLE_SHIFT;

    /* Write high nibble */
    CharLCD_WrDatNib(nibble);

    nibble = dByte & CharLCD_NIBBLE_MASK;
    /* Write low nibble */
    CharLCD_WrDatNib(nibble);
}
...
void CharLCD_WriteControl(uint8 cByte) 
{
    ...
    nibble = cByte >> CharLCD_NIBBLE_SHIFT;
    
    /* WrCntrlNib(High Nibble) */
    CharLCD_WrCntrlNib(nibble);
    nibble = cByte & CharLCD_NIBBLE_MASK;
...
void CharLCD_IsReady(void) 
{
    uint8 value;
    uint32 timeout;
    timeout = CharLCD_READY_DELAY;

    /* Clear LCD port*/
    CharLCD_PORT_DR_REG &= ((uint8)(~CharLCD_PORT_MASK));
    ...
    */
        ...
        value = CharLCD_PORT_PC_REG & ((uint32) (~ CharLCD_DM_DATA_MASK));
        /* Load in high Z values for data pins, others unchanged */
        CharLCD_PORT_PC_REG = value | CharLCD_HIGH_Z_DATA_DM;
        ...
        /* Mask off data pins to clear old values out */
        value = CharLCD_PORT_DM0_REG & ((uint8)(~CharLCD_DATA_MASK));
        /* Load in high Z values for data pins, others unchanged */
        CharLCD_PORT_DM0_REG = value | (CharLCD_HIGH_Z_DM0 & CharLCD_DATA_MASK);

        /* Mask off data pins to clear old values out */
        value = CharLCD_PORT_DM1_REG & ((uint8)(~CharLCD_DATA_MASK));
        /* Load in high Z values for data pins, others unchanged */
        CharLCD_PORT_DM1_REG = value;

        /* Mask off data pins to clear old values out */
        value = CharLCD_PORT_DM2_REG & ((uint8)(~CharLCD_DATA_MASK));
        /* Load in high Z values for data pins, others unchanged */
        CharLCD_PORT_DM2_REG = value;
    
    #endif /* CY_PSOC4 */
    
    /* Make sure RS is low */
    CharLCD_PORT_DR_REG &= ((uint8)(~CharLCD_RS));

    /* Set R/W high to read */
    CharLCD_PORT_DR_REG |= CharLCD_RW;
    ...
    do
    {
        ...
        /* Set E high */
        CharLCD_PORT_DR_REG |= CharLCD_E;

        /* 360 ns delay setup time for data pins */
        CyDelayUs(1u);

        /* Get port state */
        value = CharLCD_PORT_PS_REG;

        /* Set enable low */
        CharLCD_PORT_DR_REG &= ((uint8)(~CharLCD_E));

        /* This gives true delay between disabling Enable bit and polling Ready bit */
        CyDelayUs(0u);

        /* Extract ready bit */
        value &= CharLCD_READY_BIT;
    ...
    CharLCD_PORT_DR_REG &= ((uint8)(~CharLCD_RW));
    ...
    CharLCD_PORT_DR_REG &= ((uint8)(~CharLCD_PORT_MASK));
        ...
        value = CharLCD_PORT_PC_REG & ((uint32)(~ CharLCD_DM_DATA_MASK));
        /* Load in high Z values for data pins, others unchanged */
        CharLCD_PORT_PC_REG = value | CharLCD_STRONG_DATA_DM;
        ...
        */
        CharLCD_PORT_DM0_REG &= ((uint8)(~CharLCD_DATA_MASK));
        /* Mask off data pins to clear high z values out */
        value = CharLCD_PORT_DM1_REG & ((uint8)(~CharLCD_DATA_MASK));
        /* Configure data pins to Strong Drive, others unchanged */
        CharLCD_PORT_DM1_REG = value | (CharLCD_STRONG_DM1 & CharLCD_DATA_MASK);

        /* Mask off data pins to clear high z values out */
        value = CharLCD_PORT_DM2_REG & ((uint8)(~CharLCD_DATA_MASK));
        /* Configure data pins to Strong Drive, others unchanged */
        CharLCD_PORT_DM2_REG = value | (CharLCD_STRONG_DM2 & CharLCD_DATA_MASK);
...
/*******************************************************************************
*  Function Name: CharLCD_WrDatNib
********************************************************************************
*
* Summary:
...
static void CharLCD_WrDatNib(uint8 nibble) 
{
    ...
    CharLCD_PORT_DR_REG |= CharLCD_RS;
    ...
    /* Clear data pins */
    CharLCD_PORT_DR_REG &= ((uint8)(~CharLCD_DATA_MASK));

    /* Write data, bring E high */
    #if(0u != CharLCD_PORT_SHIFT) /* MISRA forbids shift by 0 so need to handle that */
        CharLCD_PORT_DR_REG |= 
            (CharLCD_E | ((uint8)(((uint8) nibble) << CharLCD_PORT_SHIFT)));
    #else
        CharLCD_PORT_DR_REG |= (CharLCD_E | nibble);
    #endif /* (0u != CharLCD_PORT_SHIFT) */
...
/*******************************************************************************
*  Function Name: CharLCD_WrCntrlNib
********************************************************************************
*
* Summary:
...
static void CharLCD_WrCntrlNib(uint8 nibble) 
{
    ...
    CharLCD_PORT_DR_REG &= ((uint8)(~(CharLCD_RS | CharLCD_RW)));
    ...
    *******************************************************************************/
    ...
    /*******************************************************************************
    *  Function Name: CharLCD_PrintInt32
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    {
        uint8 shift = CharLCD_32_BIT_SHIFT;
        ...
        while (shift != 0u)
        {
            ...
            */
            shift -= CharLCD_8_BIT_SHIFT;
            
            /* Print 8 bits of uint32 hex number */
            CharLCD_PrintInt8((uint8) ((uint32) (value >> shift)));
        }
    }
    ...
    *******************************************************************************/
    ...
    /*******************************************************************************
    *  Function Name: CharLCD_PrintU32Number
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    {
        uint8 tmpDigit;
        char8 number[CharLCD_NUMBER_OF_REMAINDERS_U32 + 1u];
        uint8 digIndex = CharLCD_NUMBER_OF_REMAINDERS_U32;
        ...
        */
        ...
        while(value >= CharLCD_TEN)
        {
            ...
            */
            tmpDigit = (uint8) (((uint8) (value % CharLCD_TEN)) + (uint8) CharLCD_ZERO_CHAR_ASCII);
        ...
        */
        ...
        /* Print out number */
        CharLCD_PrintString(&number[digIndex]);
    }
