...
void CharLCD_LoadCustomFonts(uint8 const customData[]) 
{
    ...
    */
    CharLCD_WriteControl(CharLCD_CGRAM_0);

    /* Load in the 64 bytes of CustomChar Data */
    for(indexU8 = 0u; indexU8 < CharLCD_CUSTOM_CHAR_SET_LEN; indexU8++)
    {
        CharLCD_WriteData(customData[indexU8]);
    }

    CharLCD_IsReady();
    CharLCD_WriteControl(CharLCD_DDRAM_0);
}
