...
void Speed_cal_Start(void) 
{
    ...
    Speed_cal_CLKEN |= Speed_cal_CLKEN_MASK;
	Speed_cal_CLKSTBY |= Speed_cal_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Speed_cal_Stop
********************************************************************************
*
* Summary:
...
void Speed_cal_Stop(void) 
{
    ...
    Speed_cal_CLKEN &= (uint8)(~Speed_cal_CLKEN_MASK);
	Speed_cal_CLKSTBY &= (uint8)(~Speed_cal_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Speed_cal_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Speed_cal_CLKEN & Speed_cal_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Speed_cal_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Speed_cal_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Speed_cal__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Speed_cal_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Speed_cal_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Speed_cal_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Speed_cal_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Speed_cal_GetSourceRegister();
    uint16 oldDivider = Speed_cal_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Speed_cal_CLKEN & Speed_cal_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Speed_cal_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Speed_cal_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Speed_cal_DIV_PTR);
}


/*******************************************************************************
* Function Name: Speed_cal_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Speed_cal_MOD_SRC |= modeBitMask & (uint8)Speed_cal_MODE_MASK;
}


/*******************************************************************************
* Function Name: Speed_cal_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Speed_cal_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Speed_cal_MODE_MASK));
}


/*******************************************************************************
* Function Name: Speed_cal_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Speed_cal_MOD_SRC & (uint8)(Speed_cal_MODE_MASK);
}


/*******************************************************************************
* Function Name: Speed_cal_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Speed_cal_GetDividerRegister();
    uint8 oldSrc = Speed_cal_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Speed_cal_MOD_SRC |= CYCLK_SSS;
        Speed_cal_MOD_SRC =
            (Speed_cal_MOD_SRC & (uint8)(~Speed_cal_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Speed_cal_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Speed_cal_MOD_SRC & Speed_cal_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Speed_cal_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Speed_cal_PHASE = clkPhase & Speed_cal_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Speed_cal_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Speed_cal_PHASE & Speed_cal_PHASE_MASK;
}
