 ...
 * host has been sent a request to change line coding or control bitmap. */
 ...
 *SET_CONTROL_LINE request and returns it to the user code using the USBUART_1_GetLineControl() API.*/
volatile uint16 USBUART_1_linesControlBitmap[USBUART_1_MAX_MULTI_COM_NUM];
/** Contains the 16-bit serial state value that was sent using the \ref USBUART_1_SendSerialState() API*/
volatile uint16 USBUART_1_serialStateBitmap[USBUART_1_MAX_MULTI_COM_NUM];
/** Contains the data IN endpoint number. It is initialized after a SET_CONFIGURATION request 
 * based on a user descriptor. It is used in CDC APIs to send data to the PC. */
volatile uint8  USBUART_1_cdcDataInEp[USBUART_1_MAX_MULTI_COM_NUM];
/** Contains the data OUT endpoint number. It is initialized after a SET_CONFIGURATION request 
 * based on user descriptor. It is used in CDC APIs to receive data from the PC.*/
volatile uint8  USBUART_1_cdcDataOutEp[USBUART_1_MAX_MULTI_COM_NUM];
/** Contains the data IN endpoint number for COMMUNICATION interface. It is initialized after a SET_CONFIGURATION request 
 * based on a user descriptor. It is used in CDC APIs to send data to the PC. */
...
***************************************/
    ...
    *  from the PC. This API should be called after the device has been started 
    *  and configured using USBUART_Start() API to initialize and start the 
    *  USBFS component operation. Then call the USBUART_GetConfiguration() API 
    *  to wait until the host has enumerated and configured the device. 
    *  For example:
    *  
    ...
    *******************************************************************************/
    void USBUART_1_PutData(const uint8* pData, uint16 length) 
    {
        ...
        if (length > USBUART_1_EP[USBUART_1_cdcDataInEp[USBUART_1_activeCom]].bufferSize)
        {
            /* Caution: Data will be lost if length is greater than Max Packet Length */
            length = USBUART_1_EP[USBUART_1_cdcDataInEp[USBUART_1_activeCom]].bufferSize;
            
            /* Halt CPU in debug mode */
            CYASSERT(0u != 0u);
        }
        
        USBUART_1_LoadInEP(USBUART_1_cdcDataInEp[USBUART_1_activeCom], pData, length);
    }
    ...
    ***************************************************************************/
    void USBUART_1_PutString(const char8 string[]) 
    {
        ...
        {
            /* Limits length to maximum packet size for the EP */
            sendLength = (strLength > USBUART_1_EP[USBUART_1_cdcDataInEp[USBUART_1_activeCom]].bufferSize) ?
                                USBUART_1_EP[USBUART_1_cdcDataInEp[USBUART_1_activeCom]].bufferSize : strLength;
             
            /* Enable IN transfer */
            USBUART_1_LoadInEP(USBUART_1_cdcDataInEp[USBUART_1_activeCom], (const uint8 *)&string[bufIndex], sendLength);
            strLength -= sendLength;

            /* If more data are present to send or full packet was sent */
            if ((strLength > 0u) || (sendLength == USBUART_1_EP[USBUART_1_cdcDataInEp[USBUART_1_activeCom]].bufferSize))
            {
                bufIndex += sendLength;
                
                /* Wait for the Host to read buffer */
                while(USBUART_1_EP[USBUART_1_cdcDataInEp[USBUART_1_activeCom]].apiEpState == USBUART_1_IN_BUFFER_FULL)
                {
                }
                ...
                */
                if (strLength == 0u)
                {
                    USBUART_1_LoadInEP(USBUART_1_cdcDataInEp[USBUART_1_activeCom], NULL, 0u);
                }
            }
        }
        while(strLength > 0u);
    ...
    *******************************************************************************/
    uint16 USBUART_1_GetCount(void) 
    {
        ...
        if (USBUART_1_EP[USBUART_1_cdcDataOutEp[USBUART_1_activeCom]].apiEpState == USBUART_1_OUT_BUFFER_FULL)
        {
            bytesCount = USBUART_1_GetEPCount(USBUART_1_cdcDataOutEp[USBUART_1_activeCom]);
        }
        else
        {
            bytesCount = 0u;
    ...
    ****************************************************************************//**
    *
    *  This function returns a nonzero value if the component received data or 
    ...
    *
    * \return
    *  If the OUT packet is received, this function returns a nonzero value. 
    ...
    *******************************************************************************/
    ...
    {
        return (USBUART_1_EP[USBUART_1_cdcDataOutEp[USBUART_1_activeCom]].apiEpState);
    }
    ...
    *******************************************************************************/
    ...
    {
        return (USBUART_1_EP[USBUART_1_cdcDataInEp[USBUART_1_activeCom]].apiEpState);
    }
    ...
    ***************************************************************************/
    ...
    {
        return (USBUART_1_ReadOutEP(USBUART_1_cdcDataOutEp[USBUART_1_activeCom], pData, length));
    }
    ...
    *******************************************************************************/
    ...
    {
        return (USBUART_1_ReadOutEP(USBUART_1_cdcDataOutEp[USBUART_1_activeCom], pData,
                                           USBUART_1_EP[USBUART_1_cdcDataOutEp[USBUART_1_activeCom]].bufferSize));
    }
    ...
    ***************************************************************************/
    uint8 USBUART_1_GetChar(void) 
    {
        ...
        (void) USBUART_1_ReadOutEP(USBUART_1_cdcDataOutEp[USBUART_1_activeCom], &rxData, 1u);
    ...
    *******************************************************************************/
    ...
    {
        if(USBUART_1_SERIAL_STATE_SIZE <= USBUART_1_EP[USBUART_1_cdcCommInInterruptEp[USBUART_1_activeCom]].bufferSize)
        {
            /*Save current SERIAL_STATE bitmap*/
            USBUART_1_serialStateBitmap[USBUART_1_activeCom] = serialState;
            /*Add interface number*/
            USBUART_1_serialStateNotification.wIndex = USBUART_1_EP[USBUART_1_cdcCommInInterruptEp[USBUART_1_activeCom]].interface;
            /*Form SERIAL_STATE data*/
            USBUART_1_serialStateNotification.wSerialState = (uint8) USBUART_1_serialStateBitmap[USBUART_1_activeCom] & (0xFFu);
            USBUART_1_serialStateNotification.wSerialStateMSB = (uint8) (USBUART_1_serialStateBitmap[USBUART_1_activeCom] >> 0x8u) & (0xFFu);
            
            USBUART_1_LoadInEP(USBUART_1_cdcCommInInterruptEp[USBUART_1_activeCom], (uint8 *)&(USBUART_1_serialStateNotification), sizeof(USBUART_1_serialStateNotification));
        }
    }
    ...
    ****************************************************************************//**
    *
    *  This function returns a nonzero value if the Component is ready to send 
    ...
    *  function returns a nonzero value. Otherwise, it returns zero.
    *
    * \globalvars
    *   USBUART_1_cdcDataInEp: CDC IN endpoint number used.
    *
    ...
    *******************************************************************************/
    ...
    {
        if ((USBUART_1_activeCom != comPortNumber) && \
                        (comPortNumber < USBUART_1_MAX_MULTI_COM_NUM ))
        {

            USBUART_1_activeCom = comPortNumber;        
        }
    }
